...
namespace Crowd.BLL
{
    ...
    {
        private readonly Crowd.DAL.userinfo dal = new Crowd.DAL.userinfo();
        public userinfo()
        { }
        #region  Method
        /// <summary>
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Crowd.Model.userinfo GetModelByCache(int id)
        {
            ...
            return (Crowd.Model.userinfo)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Crowd.Model.userinfo> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Crowd.Model.userinfo> DataTableToList(DataTable dt)
        {
            List<Crowd.Model.userinfo> modelList = new List<Crowd.Model.userinfo>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Crowd.Model.userinfo model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Crowd.Model.userinfo();
                    ...
                    if (dt.Rows[n]["headImg"] != null && dt.Rows[n]["headImg"].ToString() != "")
                    {
                        model.headImg = int.Parse(dt.Rows[n]["headImg"].ToString());
                    }
                    modelList.Add(model);
                }
            }
